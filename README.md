...
Bog standard utilities and mixins for the javascript ecosystem.

## Getting Started

This project is broken down into multiple packages each targetted for a specific platform or a framework.
...
4. `@jabascript/query`: Utilities to create and manipulate URLs.
...
To start development, clone the repository and run the following commands:

```sh
pnpm i
pnpm exec playwright install chromium --with-deps # Only needed for running tests
```
